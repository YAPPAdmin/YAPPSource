<script lang="ts">
    ...
    import { formatDate, formatTime, getDayFromDate, getTimeFromDate } from "$lib/utils/util";
    import Tooltip from "$lib/svelteComponents/base/Tooltip.svelte";
    import type { BlogPostN, BlogPostVersion } from "$lib/utils/blog";
    import { User } from "$lib/utils/auth/User";


    let blogpost: BlogPostN | undefined = undefined;
    let authorCache: any[] = [];
    ...
    let activeVersion: BlogPostVersion | undefined = undefined;
    ...
    onMount(() => {
        blogpost = page.data.post;
        authorCache = page.data.authorCache;
		user = User.fromJSON(page.data.user);


        if(!blogpost) return;

        groupedVersions = Object.entries(
            blogpost.versions.reduce((acc, vers) => {
                const date = getDayFromDate(vers.updatedAt);
        ...
        // Set active version
        activeVersion = blogpost.versions.find(version => version.active);
        ...
        <div class="py-8 lg:pe-8">
            <div class="space-y-5 lg:space-y-8">
				...
				<div class="w-full flex justify-between items-center">
					<!-- Titel -->
					<h1 class="text-3xl font-bold lg:text-5xl">{activeVersion.title}</h1>

					<!-- Edit Button for Admins -->
					{#if user && user.getRole("admin") || user.getRole("moderator")}
						<Tooltip text="Edit Blogpost">
							<a href={`/settings/blogeditor/${blogpost.id}`}>
								<Iconify iconId={"material-symbols-light:edit-outline-rounded"} button={true} height={36} />
							</a>
						</Tooltip>
					{/if}
				</div>
				...
				<div class="flex justify-between items-center">
					...
					<div class="space-x-4">
						<a class="inline-flex items-center gap-1.5 py-1 px-3 sm:py-2 sm:px-4 rounded-full text-xs sm:text-sm bg-secondary-900 text-gray-800 hover:bg-secondary-800 focus:outline-hidden focus:bg-gray-200" href="#">
							{activeVersion.category}
						</a>

						<!-- DROPDOWN FOR VERSION SELECT -->


					</div>


					<p class="text-xs sm:text-sm text-gray-800">{formatDate(activeVersion.updatedAt)}</p>
            ...
            <!-- Blog Post Content -->
                ...
                <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-y-5 lg:gap-y-0">
                    ...
                    <!-- /Tags -->
                    <div>
                        {#each activeVersion.tags as tag}
                    ...
                    <div class="flex justify-end items-center gap-x-1.5">
                        ...
                        <div class="hs-tooltip inline-block">
                            <button type="button" class="hs-tooltip-toggle flex items-center gap-x-2 text-sm text-gray-500 hover:text-gray-800 focus:outline-hidden focus:text-gray-800">
                                <Iconify iconId={"material-symbols-light:thumb-up-outline-rounded"} inline={true} button={true}/>
...
{:else if activeVersion == undefined}
    <p>Loading</p>
{:else}
    <h1>This blog post <strong class="!text-secondary-500 font-medium">(yet)</strong> doesn't exist!</h1>
{/if}
